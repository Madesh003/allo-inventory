...
/**
 * Expires stale PENDING reservations and releases their held stock.
 *
 * Two-layer strategy:
 *  1. Lazy cleanup on read/write paths — called before stock queries and before reservations
 *  2. Vercel Cron job every minute — ensures expiry happens even with no traffic
 */
export async function expireStaleReservations(): Promise<number> {
  ...
  // Release each reservation status
  ...
  // Group inventory updates to batch them
